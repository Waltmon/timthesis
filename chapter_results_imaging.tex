...
619-nm fluorescence signal is plotted vs. average ions in the laser region in Fig. \ref{fig:lin} for several deposits on two different days.  The bi-convex laser-focusing lens was used in Fig. \ref{fig:lin}(a), and the aspherical lens in Fig. \ref{fig:lin}(b).  Signal was determined by summing the counts of a 3-pixel$\times$3-pixel (15$\mu$m$\times$15$\mu$m) region centered on the laser spot in the image, and background was determined by averaging the 3-pixel$\times$3-pixel sum of the Xe-only runs before and after each Ba\textsuperscript{+} \emph{\color{gray}may want to put sub-pixel integrator in Method}.  Signal and background were scaled to laser power respective to each run, and then background was subtracted.  [describe what's in the plot ... hopefully it's a better one, but could show the combined one in addition to.   ...mention 0-pulse]
...
Subtracted images of 619-nm Ba fluorescence are shown in Fig. \ref{fig:train} for a few deposits near the linear trend line in Fig. \ref{fig:lin}(b).
