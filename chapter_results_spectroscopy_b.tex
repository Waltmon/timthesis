...
\begin{figure} %[H]
        \centering
                ...
                \caption{\color{gray}may want to show 3, though 1 I think was bad.}
...
\begin{figure} %[H]
        \centering
                ...
                \caption{\emph{\color{gray}should separate violets by power in legend}}
\label{fig:bleach_repump}
\end{figure}

Fluorescence counts vs. time for several Ba\textsuperscript{+} deposits are shown in Fig. \ref{fig:bleach_repump} for several laser combinations, all of which were combined by dichroic filters into the same path.  Each curve is a separate deposit, all of which are scaled to begin at the same point for comparison.  In Fig. \ref{fig:bleach_repump}(a), green-only (555~nm) excitation (i.e., no re-pump lasers) is shown along with green + IR (1064~nm and 1550~nm), green + IR + red (657~nm), green + IR + blue (C480 dye at ???~nm), and green + IR + red + blue.  Increased bleaching was observed by inclusion of these re-pump lasers, especially with the blue laser.  In Fig. \ref{fig:bleach_repump}(b), green-only is shown again along with green + IR and green + IR + violet (Kr ion laser at 406~nm).  Here, {\color{gray}the IR lasers we at a lower power than in (a), and no difference is observed vs. green-only.}  Increased bleaching was observed with the violet laser, with more bleaching at the higher violet laser power.
...
The 619-nm peak does bleach somewhat at much higher laser intensities.  At a few mW of around 570~nm, bleaching is observed when the laser is focused.  Integrated 619-nm fluorescence, divided by laser power, vs. time from an image of a focused 570-nm laser is shown in Fig. \ref{fig:bleaching619}.  Here, three scalings of laser power and exposure time are used such that $I \times t$ is conserved, e.g. the curve with 0.21~mW laser power (green) has 1/3 the exposure time of that with 0.07~mW (blue).  \emph{\color{gray}think one more time... these are scaled down, right?  THAT'S IMPORTANT TO MENTION}... indicates a time-dependent return mechanism for the fluorescence.  Faster exposure times are more convenient, however this must be balanced against a loss of signal-to-noise.  The 0.21~mW laser power was chosen, corresponding to an intensity of about 11.8~$\mu$W/$\mu$m$^{2}$ \emph{\color{gray}what was that based on?  say ``average" due to vibrations?}.
...
\begin{figure} %[H]
        \centering
                ...
                \caption{Comparison of Ba\textsuperscript{+} deposits made at (a) 55~K vs. 11~K, and (b) 20~nm/s vs. 0.7~nm/s \emph{\color{gray}Are those rates corrected?}.\cite{Mong2015}}
\label{fig:BaHx}
\end{figure}
