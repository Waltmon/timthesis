...
%        %\end{subfigure}
        %\begin{subfigure}
                ...
                \caption{\emph{\color{red}Needs (a),(b)} Raw (left) and shaped (right) pulse signals from induction plates and cup 3.  The raw induction signal appears small because it is a less sensitive pre-amp (accounted for in shaping).}
        %\end{subfigure}
...
A BaAl$_{4}$ getter is employed as a purely neutral Ba source which can be inserted on a bellows to emit toward the sapphire window, shown in Fig. \ref{fig:endOfBeam}.  Getters were used extensively in previous work \cite{Brian} for measuring the absorption of Ba in SXe with large Ba deposits, and in identifying emission peaks.  A getter is used briefly in this work in identifying the 619-nm fluorescence peak, as described in Section \ref{sec:619identification}.  Getters used in \cite{Brian} were exothermic BaAl$_{4}$-Ni flash getters.  The getter used here is an endothermic type, designed for more controlled Ba emission.  
